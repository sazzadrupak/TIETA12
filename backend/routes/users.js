...
router.post('/signup', [validator(validate)], async (req, res) => {
  ...
  user = new User(_.pick(req.body, ['firstName', 'lastName', 'userType', 'bankName', 'accountNo', 'email', 'password']));
...
router.post('/login', async (req, res) => {
  ...
  if (match) {
    ...
    return res.header('x-auth-token', token).send(_.pick(user, ['_id', '_links']));
  }
...
router.get('/:id', [auth, validateObjectId], async (req, res) => {
  ...
  if (userType === 'admin' || _id === req.params.id) {
    ...
    return res.send(_.pick(user, ['firstName', 'lastName', 'userType', 'bankName', 'accountNo', 'email', '_links']));
  }
...
router.put('/:id', [auth, validateObjectId], asyncMiddleware(async (req, res) => {
  ...
  if (userType === 'admin' || _id === req.params.id) {
    ...
    try {
      const updateUser = await User.findByIdAndUpdate(req.params.id, updateBody, { new : true });
      return res.send(_.pick(updateUser, ['firstName', 'lastName', 'userType', 'bankName', 'accountNo', 'email', '_links']));
    } catch (ex) {
      const { code, message } = mongooseError(ex);
      return res.status(code).send(message);
    }
...
router.put('/userType/:id', [auth, validateObjectId], asyncMiddleware(async (req, res) => {
